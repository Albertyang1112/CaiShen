...
function TxDetailModal({ tx, bankAccounts, coa, coaById, onUpdate, reload, onClose }) {
  ...
  const save = async () => {
    setSaving(true)
    try {
      await axios.patch(`${API}/transactions/${tx.id}`, { coaId: coaId || null, note })
      // Functional update so a concurrent SSE refetch doesn't get mapped over a stale snapshot.
      onUpdate(prev => prev.map(t => t.id === tx.id ? { ...t, coaId: coaId || null, note } : t))
...
export default function Banking({ accounts, transactions, onUpdate }) {
  ...
  const statusCounts = {
    ...
    posted:  scopedTxs.filter(t => !t.pending).length,
    pending: scopedTxs.filter(t =>  t.pending).length,
  }

  // status tab + sort → filteredTxs
  let filteredTxs = statusFilter === 'pending' ? scopedTxs.filter(t =>  t.pending)
                  : statusFilter === 'posted'  ? scopedTxs.filter(t => !t.pending)
                  : [...scopedTxs]
        ...
        <div>
          ...
          {view==='transactions' && (
            <div>
              ...
              {/* Status tabs (All / Posted / Pending) */}
              <div style={{display:'flex',gap:6,marginBottom:14,alignItems:'center'}}>
                {[['all','All'],['posted','Posted'],['pending','Pending']].map(([id,label]) => (
