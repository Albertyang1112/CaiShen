...
### Mortgage (`client/src/pages/Mortgage/Mortgage.jsx`, July 2026)
Servicer-portal style page (what Rocket Mortgage / Mr. Cooper / Fay Servicing show a borrower) over the mortgage domain tables. Read-only — data arrives automatically from two complementary sources:
1. **Statement PDFs** — `banking/mortgage-parse.js` (label-gap regexes, same-line only, plus a column-aware fallback `grabByColumn` that requires the value to sit within ±20 chars of its label's column — handles both Rocket's value-below-label and Mr. Cooper's interleaved two-column pdftotext layouts) → `banking/mortgage.js` recorder. New statements parse during scraper import; already-ingested ones via `node server/scripts/backfill-mortgage-statements.js [userId] [--apply]` (dry-run default, idempotent, records chronologically, keeps only the newest statement's alerts).
2. **Plaid Liabilities** — `banking/liabilities.js` on every sync (needs the "Loan data" consent button in Connections); fills rate_type/loan_term/YTD/PMI/property-address columns.
Both writers look up an existing `mortgage_accounts` row by `account_id` OR `loan_number_mask` before inserting, so one physical loan never splits into two rows.
UI: hero metrics (principal, payment, rate, escrow), principal-paydown SVG chart + client-side amortization payoff projection, payment history with P/I/E split + matched-bank-txn badge + statement-PDF preview modal (auth fetch → pdfjs-dist canvas pages, like the Data Vault preview — Chrome does NOT reliably render blob-URL PDFs in an iframe), escrow activity (hidden when empty), alerts strip. Multi-property: a dropdown (top-right, hidden for a single loan) switches between loans, each labeled by its property — name from `/api/properties` when `property_id` is linked, else the statement's printed street, else servicer + loan last-4. Loan number is masked to last-4 by default; an eye toggle reveals the full number (`mortgage_accounts.loan_number`, filled by both the statement parser and Plaid). Live market rates: `banking/market-rates.js` pulls Freddie Mac PMMS 30/15-yr weekly averages from FRED's keyless CSV endpoint, cached in-process 6h → `GET /api/mortgage/market-rates`; the rate card compares the loan's rate to market automatically. What-if payments: the P&I figure in the paydown card is editable — the projection re-amortizes live with a vs-actual comparison (months sooner, interest saved); a floppy icon saves the scenario per-loan (`PUT /api/mortgage/:id/whatif` → per-user `mortgage_prefs.json`, survives reload), an arrow icon reverts an edit or clears a saved scenario back to the actual last-month payment. Nav id `mortgage` (Overview section). API: `GET /api/mortgage`, `/api/mortgage/:id/statements`, `/api/mortgage/alerts`, `/api/mortgage/market-rates`, `PUT /api/mortgage/:id/whatif`.

### Spreadsheet Import (`server/imports/`, July 2026)
Bulk QuickBooks-export (and clearly-labeled generic .xlsx/.csv) import — "Import data" button on the Banking page (also in the no-accounts empty state; drop target + file picker). Files are parsed ENTIRELY in memory (multer memoryStorage + exceljs buffer load) and **never stored** — only extracted data + a sha256 batch record (`qb_import_batches.json`, last 20) persist.
- **Pipeline** (`imports/importer.js`): classify workbooks (`classify.js` — QB banner-title rules first, header-keyword sniff for generic sheets, one small Groq call ONLY for sheets rules can't read) → build account/chart plan from Trial Balance + Balance Sheet + P&L (`qb-chart.js` — BS sections decide bank/card vs asset/liability/equity; P&L decides income/expense) → convert the **Journal** (`qb-journal.js`) → verify → write. The **General Ledger is deliberately ignored** (repeats each txn per account; would double-count) and Balance Sheet/P&L/Trial Balance are never imported as data — they are the verification answer key.
- **Conversion rule**: a display txn exists for every Journal leg on a real bank/CC account; the opposite leg supplies `coaId`/category. 1 fin leg + N category legs → N txns (QB splits stay exact, `isSplit`). ≥2 fin legs → linked transfer pair (`transferGroup`, category 'Transfer', no coaId — auto-categorize excludes from reports). 0 fin legs → accounting journal entry (`journal_entries.json`, feeds /pl). Amount = debit − credit (bank deposit +, CC purchase −, matching Plaid convention).
- **Accounts**: matched by last-4 (then exact name) before creating — one physical account never splits. Created rows: `source:'quickbooks'`, id `qb_acct_<slug>`, balance from Trial Balance (cards flipped to owed-positive). QB non-bank accounts become `system:false` chart nodes (`cat_qb_<slugpath>`, survive loadChart self-healing) under the matching default root by type+scope (scope: 'personal' iff QB name says personal, else 'business'); their TB balances land in `category_balances.json` (equity kept credit-normal signed so Owner draws reads negative like QB).
- **Provenance**: every txn gets `source_transactions` (source 'quickbooks', raw = full parsed row) + evidence link (role 'quickbooks' in matching.js ROLES). Importer awaits `store.flush()` (drains the fire-and-forget table mirrors) before provenance + response — the response triggers UI refetches that read the transactions TABLE, and a mid-mirror read atomically sees the old set ("0 transactions" race); provenance also FKs onto accounts. Txn ids are content hashes (date|account|amount|memo|occurrence) → **re-import is idempotent** (replace-by-id).
- **Balance sheet Equity section**: Accounting.jsx now itemizes equity-type chart leaves + a computed "Retained earnings + net income" residual line; Total Equity = assets − liabilities (identity).
- **Entity merge (Albert's standing policy)**: imported data must reconcile against everything already in the app — same entity = ONE record, never duplicated, never summed. Mortgages: `core/loan-match.js` matches a linked `mortgage_accounts` row to its book leaf by property name ("8962 Kobe Pl" ↔ "Kobe Mortgage", adjacent-word joins handle "Bay Hill"↔"Bayhill"); importer skips/heals the book category balance and flags book-vs-servicer drift; `/balance-sheet` itemizes the live loan ON the book leaf (inherits the books' Business section — the hardcoded 'Personal > Primary Mortgage' leaf is only the no-match fallback). Credit cards on `/balance-sheet` are SIGNED (owed +, credit balance −, netting like QB), not abs().
- **Verify/debug**: `node server/scripts/qb-import-dryrun.js <dir>` (no writes); `POST /api/import/spreadsheets?dryRun=1`; tests in `server/tests/spreadsheetImport.test.js`. Against Albert's real 8-file export: 29 accounts, 10,232 txns, 658 transfer pairs, 45 JEs — every account and every P&L category reconciled exactly.
- **Deferred (remind Albert)**: QB Bills/Invoices exports (A/P detail), customer/vendor contact details + tenant roster (names DO seed vendor_memory).
