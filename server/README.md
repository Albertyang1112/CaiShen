...
| **banking/** | `/api` (routes.js), `/api/reconcile`, `/api/receipts` | The Banking page + money pipeline. `routes.js` (accounts, transactions, tx-overrides, categorization-rules), `plaid.js` (live sync + webhook + auto-reconcile), `statements.js`, `reconciler.js` + `reconcile-routes.js` (statement↔Plaid matching), `receipt-routes.js` + `receipt-ocr.js`, `categorize.js` (rule engine) + `categorize-ai.js`, `notifier.js`, `neon-mirror.js` (Plaid→Neon), `discord-bot.js` (reply-to-categorize). |
